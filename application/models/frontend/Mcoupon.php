<?php  
	class Mcoupon extends DB{
		public function select_coupon($trash,$start,$limit){
			$qr = "SELECT * FROM coupon WHERE trash=$trash order by created_at desc limit $start,$limit ";
	 	...
	 	// index coupon num
		public function select_coupon_num($trash){
			$run = mysqli_query($this->con, "SELECT * FROM coupon WHERE trash=$trash");
			$num = mysqli_num_rows($run);
			return $num;
	 	}
	 	// index coupon
		public function select_coupon_status($trash){
	 	...
	 	//insert coupon
	 	public function insert_coupon($code, $discount, $limit_number, $expiration_date, $payment_limit, $description, $created_at,  $trash, $status){
	 		$qr = "INSERT INTO coupon(code, discount, limit_number, expiration_date, payment_limit, description, created_at, trash, status) VALUES ('".$code."',".$discount.",".$limit_number.",'".$expiration_date."',".$payment_limit.",'".$description."','".$created_at."',".$trash.",".$status.")";
			$run = mysqli_query($this->con, $qr);
			return $run;
	 	}
