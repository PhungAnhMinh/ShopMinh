<?php 
	class Mproduct extends DB{
		// index product
		public function select_product($trash){
			$qr = "SELECT pt.*, c.name as 'name_category',pr.name as'name_producer' FROM ((product pt LEFT JOIN category c ON pt.category_id=c.id) LEFT JOIN producer pr ON pt.producer_id=pr.id) WHERE  c.status =1 and pt.trash=".$trash;
			...
			return array('product'=>$product,'num'=>$num);;
	 	}

	 	//insert product
	 	public function insert_product($category_id, $name, $alias, $thumbnail, $img, $description, $detail, $producer_id, $number, $sale, $price, $price_sale, $created, $created_by, $status, $trash){
	 		$qr = "INSERT INTO product (category_id, name, alias, thumbnail, img, description, detail, producer_id, number, sale, price, price_sale, created, created_by, status, trash) VALUES (".$category_id.",'".$name."','".$alias."','".$thumbnail."','".$img."','".$description."','".$detail."',".$producer_id.",".$number.",".$sale.",".$price.",".$price_sale.",'".$created."','".$created_by."',".$status.",1)";
			$run = mysqli_query($this->con, $qr);
			return $run;
	 	}
