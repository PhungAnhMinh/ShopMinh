...
// check number
	...
	// làm sạch email
	public function filterEmail($post_email){
		        // Làm sạch tên người dùng
			    $field = filter_var(trim($post_email), FILTER_SANITIZE_EMAIL);
			    // Xác thực tên người dùng
			    if(filter_var($field, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
						return $username = str_replace(" ", "", $field);
				}else{
			        return FALSE;
			    }
	}
	...
	// làm sạch string
	public function filterString($post_String){
			 // Làm sạch tên người dùng
			    $field = filter_var(trim($post_String), FILTER_SANITIZE_EMAIL);
			    // Xác thực tên người dùng
			    if(filter_var($field, FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/^[a-zA-Z\s]+$/")))){
						return $username = str_replace(" ", "", $field);
				}else{
